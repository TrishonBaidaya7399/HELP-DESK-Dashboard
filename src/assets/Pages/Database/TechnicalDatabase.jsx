  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const operatorUsers = response.data.filter(user => user?.role === "technical");
  ...
  return (
    <div className="pt-6 pb-12 mb-auto">
      ...
      <div className="my-4 grid grid-cols-3 bg-[#55D6C2] text-back w-full rounded-sm">
        ...
        <Link className="border-r-2 border-black text-center text-xl" to="/database/operator">
        <button >
          Operation Team
        </button>
        </Link>
        <Link className="border-r-2 border-black text-center text-xl text-white bg-[#296EF2]" to="/database/technical">
        <button >
          Technical Support
        </button>
        </Link>
